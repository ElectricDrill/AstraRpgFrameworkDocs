...
3. **Level and experience.** `EntityCore.Level.SetTotalCurrentExp(savedExperience)`. This is the restore entry point for the level: it clamps the experience, derives the target level from the growth formula, and drives the level there through the normal setter, so every dependent component reacts exactly as it does in play. It also grows the level-derived attribute point budget. The whole transition raises a single level-up / level-down event, not one per level crossed: restoring level 1 → 50 raises one event with `AbsAmount = 49` and runs one bulk stat and attribute pass. If you need to react per level (one reward or fanfare each), loop over `AbsAmount` yourself.
